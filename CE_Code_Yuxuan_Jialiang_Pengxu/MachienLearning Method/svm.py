from unittest import result
import numpy as np
from scipy.stats import multivariate_normal
import matplotlib.pyplot as plt
from sklearn.discriminant_analysis import LinearDiscriminantAnalysis
import joblib
from sklearn.decomposition import PCA
from sklearn import svm 

sbox=(
    ) 
hw = [bin(x).count("1") for x in range(256)]

def cov(x, y):
    return np.cov(x, y)[0][1]

corr = []
for dig in range(16):
    tempTraces = np.load('./train/traces.npy')
    tempPText  = np.load('./train/textin.npy')
    tempKey    = np.load('./train/keylist.npy')

    atkTraces = np.load("./test/2019.04.03-14.31.53_traces.npy")
    atkPText  = np.load("./test/2019.04.03-14.31.53_textin.npy")
    atkKey    = np.load("./test/2019.04.03-14.31.53_keylist.npy")

    tempSbox = [sbox[tempPText[i][dig] ^ tempKey[i][dig]] for i in range(len(tempPText))]  
    tempHW   = [hw[s] for s in tempSbox]

    atkSbox = [sbox[atkPText[i][dig] ^ atkKey[i][dig]] for i in range(len(atkPText))] 
    atkHW   = [hw[s] for s in atkSbox]


    #low pass filter 效果有提高
    import scipy.signal as signal
    bbbb, aaaa = signal.butter(8, 0.3, 'lowpass')
    tempTraces = signal.filtfilt(bbbb, aaaa, tempTraces)
    atkTraces = signal.filtfilt(bbbb, aaaa, atkTraces)


    #----------------------------------------------------------- 
    # train model  
    svc = svm.SVC(kernel='linear',C=500,gamma=0.005, decision_function_shape='ovo')
    svc.fit(tempTraces,tempHW)
    joblib.dump(svc, "./svc_best.m")
    '''
    #------------------------------------------------------------
    #load model
    svc = joblib.load( "./svc_best.m")
    #------------------------------------------------------------
    '''
    print("dimention:"+str(atkTraces.shape))

    print("correctness:"+str(svc.score(atkTraces,atkHW)))

    data = []
   
    gussHW = np.zeros((256,atkTraces.shape[0]))
    svc_gussHW = list(svc.predict(atkTraces))
    for k in range(256):
        gussSbox = [sbox[atkPText[i][dig] ^ k] for i in range(len(atkPText))] 
        gussHW[k,:]  = [hw[s] for s in gussSbox]
    for p in range(atkTraces.shape[0]):
        maybe = np.zeros(256)
        for n in range(256):
            maybe[n] = np.sum(abs(gussHW[n,0:p] - svc_gussHW[0:p]))
        index = (np.where(maybe==np.min(maybe)))
        
        if len(index[0])==1 :
            
            print("num traces"+str(p))
            data.append(1)
        else:
            data.append(0)

    print("key:"+str(index[0]))
    corr.append(data)
            
    print("matrix:"+str(abs(gussHW[index,0:p] - svc_gussHW[0:p])))

print(corr)

rate = []
corr_list = np.zeros(16)
for i in range(100):
    for j in range(16):
        if corr[j][i] == 1:
            corr_list[j] = 1
    rate.append(corr_list.sum()/16*100)

print(rate)